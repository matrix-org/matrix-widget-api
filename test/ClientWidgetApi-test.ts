...
} from "../src/interfaces/WidgetApiAction";
...
function processCustomMatrixError(
  e: unknown,
): IWidgetApiErrorResponseDataDetails | undefined {
...
describe("ClientWidgetApi", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .mocked(driver.validateCapabilities)
      .mockImplementation(async () => new Set(capabilities));
  });
  ...
  describe("receiving room state", () => {
    it("syncs initial state and feeds updates", async () => {
      ...
      const joinRules = new Promise<void>(
        (resolve) => (resolveJoinRules = resolve),
      );

      driver.readRoomState.mockImplementation(
        async (rId, eventType, stateKey) => {
          if (rId === roomId) {
  ...
  describe("send_to_device action", () => {
    it("sends unencrypted to-device events", async () => {
      ...
      await loadIframe([
        `org.matrix.msc3819.send.to_device:${event.data.type}`,
      ]);
    ...
    it("fails to send to-device events with any event type", async () => {
      ...
      await loadIframe([
        `org.matrix.msc3819.send.to_device:${event.data.type}_different`,
      ]);
    ...
    it("should reject requests when the driver throws an exception", async () => {
      driver.sendToDevice.mockRejectedValue(
        new Error(
          "M_FORBIDDEN: You don't have permission to send to-device events",
        ),
      );
  ...
  describe("get_openid action", () => {
    it("gets info", async () => {
      ...
      expect(driver.askOpenID).toHaveBeenCalledWith(
        expect.any(SimpleObservable),
      );
    });
  ...
  describe("org.matrix.msc2876.read_events action", () => {
    ...
    it("reads state events with any state key", async () => {
      ...
      await loadIframe([
        "org.matrix.msc2762.receive.state_event:net.example.test",
      ]);
    ...
    it("reads state events with a specific state key", async () => {
      ...
      await loadIframe([
        "org.matrix.msc2762.receive.state_event:net.example.test#B",
      ]);
      clientWidgetApi.setViewedRoomId("!room-id");
  ...
  describe("org.matrix.msc3869.read_relations action", () => {
    ...
    it("should reject requests when the driver throws an exception", async () => {
      driver.readEventRelations.mockRejectedValue(
        new Error(
          "M_FORBIDDEN: You don't have permission to access that event",
        ),
      );
  ...
  describe("org.matrix.msc4039.download_file action", () => {
    it("should handle and process the request", async () => {
      ...
      expect(driver.downloadFile).toHaveBeenCalledWith(
        "mxc://example.com/test_file",
      );
    });
  ...
  it("updates language", () => {
    ...
    expect(transport.send).toHaveBeenCalledWith(
      WidgetApiToWidgetAction.LanguageChange,
      { lang: "tlh" },
    );
  });
});

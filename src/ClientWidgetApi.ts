...
} from "./interfaces/IWidgetApiResponse"
import {
...
export class ClientWidgetApi extends EventEmitter {
    ...
    private pushRoomStateResult = new Map<
        string,
        Map<string, Map<string, IRoomEvent>>
    >()
    ...
    public canSendRoomEvent(
        eventType: string,
        msgtype: string | null = null,
    ): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsRoomEvent(EventDirection.Send, eventType, msgtype),
        )
    }

    public canSendStateEvent(eventType: string, stateKey: string): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsStateEvent(EventDirection.Send, eventType, stateKey),
        )
    }

    public canSendToDeviceEvent(eventType: string): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsToDeviceEvent(EventDirection.Send, eventType),
        )
    }

    public canReceiveRoomEvent(
        eventType: string,
        msgtype: string | null = null,
    ): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsRoomEvent(EventDirection.Receive, eventType, msgtype),
        )
    }

    public canReceiveStateEvent(
        eventType: string,
        stateKey: string | null,
    ): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsStateEvent(EventDirection.Receive, eventType, stateKey),
        )
    }

    public canReceiveToDeviceEvent(eventType: string): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsToDeviceEvent(EventDirection.Receive, eventType),
        )
    }

    public canReceiveRoomAccountData(eventType: string): boolean {
        return this.allowedEvents.some((e) =>
            e.matchesAsRoomAccountData(EventDirection.Receive, eventType),
        )
    }
    ...
    private beginCapabilities(): void {
        ...
        this.transport
            .send<IWidgetApiRequestEmptyData, ICapabilitiesActionResponseData>(
                WidgetApiToWidgetAction.Capabilities,
                {},
            )
            .then((caps) => {
                requestedCaps = caps.capabilities
                return this.driver.validateCapabilities(
                    new Set(caps.capabilities),
                )
            })
            .then((allowedCaps) => {
    ...
    private allowCapabilities(allowed: string[], requested: string[]): void {
        console.log(
            `Widget ${this.widget.id} is allowed capabilities:`,
            allowed,
        )

        for (const c of allowed) this.allowedCapabilities.add(c)
        const allowedEvents =
            WidgetEventCapability.findEventCapabilities(allowed)
        this.allowedEvents.push(...allowedEvents)

        this.transport
            .send(WidgetApiToWidgetAction.NotifyCapabilities, <
                INotifyCapabilitiesActionRequestData
            >{
                requested,
        ...
        if (
            ...
            !this.canUseRoomTimeline(this.viewedRoomId)
        ) {
    ...
    private handleContentLoadedAction(
        action: IContentLoadedActionRequest,
    ): void {
    ...
    private handleCapabilitiesRenegotiate(
        request: IRenegotiateCapabilitiesActionRequest,
    ): void {
        // acknowledge first
        this.transport.reply<IWidgetApiAcknowledgeResponseData>(request, {})

        const requested = request.data?.capabilities || []
        const newlyRequested = new Set(
            requested.filter((r) => !this.hasCapability(r)),
        )
        ...
        this.driver
            ...
            .then((allowed) =>
                this.allowCapabilities([...allowed], [...newlyRequested]),
            )
    }
    ...
    private handleNavigate(request: INavigateActionRequest): void {
        ...
        if (
            !request.data?.uri ||
            !request.data?.uri.toString().startsWith("https://matrix.to/#")
        ) {
            return this.transport.reply<IWidgetApiErrorResponseData>(request, {
        ...
        try {
            this.driver
                ...
                .then(() => {
                    return this.transport.reply<IWidgetApiAcknowledgeResponseData>(
                        request,
                        {},
                    )
                })
    ...
    private handleOIDC(request: IGetOpenIDActionRequest): void {
        ...
        const replyState = (
            ...
            if (phase > 1) {
                ...
                return this.transport.reply<IGetOpenIDActionResponseData>(
                    request,
                    {
                        state: state,
                        ...credential,
        ...
        const replyError = (
            ...
            if (phase > 1) {
                ...
                return this.transport.reply<IWidgetApiErrorResponseData>(
                    request,
                    {
                        error: { message: msg },
                    },
        ...
        const observer = new SimpleObservable<IOpenIDUpdate>((update) => {
            if (
                update.state === OpenIDRequestState.PendingUserConfirmation &&
                phase > 1
            ) {
                observer.close()
                return replyError(
    ...
    private handleReadRoomAccountData(
        request: IReadRoomAccountDataFromWidgetActionRequest,
    ): void | Promise<void> {
        ...
        return events.then((evs) => {
            this.transport.reply<IReadRoomAccountDataFromWidgetResponseData>(
                request,
                { events: evs },
            )
        })
    ...
    private async handleReadEvents(
        request: IReadEventFromWidgetActionRequest,
    ): Promise<void> {
        ...
        if (
            request.data.limit !== undefined &&
            (!request.data.limit || request.data.limit < 0)
        ) {
            return this.transport.reply<IWidgetApiErrorResponseData>(request, {
        ...
        if (request.data.room_ids === undefined) {
            ...
            askRoomIds = this.driver
                .getKnownRooms()
                .filter((roomId) => this.canUseRoomTimeline(roomId))
        } else {
        ...
        if (request.data.state_key !== undefined) {
            stateKey =
                request.data.state_key === true
                    ? undefined
                    : request.data.state_key.toString()
            if (
                !this.canReceiveStateEvent(request.data.type, stateKey ?? null)
            ) {
                return this.transport.reply<IWidgetApiErrorResponseData>(
                    request,
                    {
    ...
    private handleSendEvent(request: ISendEventFromWidgetActionRequest): void {
        ...
        if (
            ...
            !this.canUseRoomTimeline(request.data.room_id)
        ) {
            return this.transport.reply<IWidgetApiErrorResponseData>(request, {
                error: {
        ...
        const isDelayedEvent =
            request.data.delay !== undefined ||
            request.data.parent_delay_id !== undefined
        if (
            isDelayedEvent &&
            !this.hasCapability(MatrixCapabilities.MSC4157SendDelayedEvent)
        ) {
            return this.transport.reply<IWidgetApiErrorResponseData>(request, {
                error: { message: "Missing capability" },
            })
        ...
        let sendEventPromise: Promise<
            ISendEventDetails | ISendDelayedEventDetails
        >
        if (request.data.state_key !== undefined) {
            if (
                !this.canSendStateEvent(
                    request.data.type,
        ...
        sendEventPromise
            .then((sentEvent) => {
                return this.transport.reply<ISendEventFromWidgetResponseData>(
                    request,
                    {
    ...
    private handleUpdateDelayedEvent(
        request: IUpdateDelayedEventFromWidgetActionRequest,
    ): void {
    ...
    private async handleSendToDevice(
        request: ISendToDeviceFromWidgetActionRequest,
    ): Promise<void> {
        if (!request.data.type) {
            ...
            try {
                await this.driver.sendToDevice(
                    request.data.type,
                    request.data.encrypted,
                    request.data.messages,
                )
                await this.transport.reply<ISendToDeviceFromWidgetResponseData>(
                    request,
                    {},
                )
            } catch (e) {
    ...
    private async pollTurnServers(
        turnServers: AsyncGenerator<ITurnServer>,
        initialServer: ITurnServer,
    ): Promise<void> {
        try {
    ...
    private async handleWatchTurnServers(
        request: IWatchTurnServersRequest,
    ): Promise<void> {
        if (!this.hasCapability(MatrixCapabilities.MSC3846TurnServers)) {
            ...
            try {
                ...
                await this.transport.reply<IWidgetApiErrorResponseData>(
                    request,
                    {
    ...
    private async handleUnwatchTurnServers(
        request: IUnwatchTurnServersRequest,
    ): Promise<void> {
        if (!this.hasCapability(MatrixCapabilities.MSC3846TurnServers)) {
            await this.transport.reply<IWidgetApiErrorResponseData>(request, {
                error: { message: "Missing capability" },
    ...
    private async handleReadRelations(
        request: IReadRelationsFromWidgetActionRequest,
    ): Promise<void> {
        ...
        if (
            request.data.room_id !== undefined &&
            !this.canUseRoomTimeline(request.data.room_id)
        ) {
            return this.transport.reply<IWidgetApiErrorResponseData>(request, {
                error: {
            ...
            const chunk = result.chunk.filter((e) => {
                if (e.state_key !== undefined) {
                    ...
                    return this.canReceiveRoomEvent(
                        e.type,
                        (e.content as { msgtype?: string })["msgtype"],
                    )
                }
            })
    ...
    private async handleUserDirectorySearch(
        request: IUserDirectorySearchFromWidgetActionRequest,
    ): Promise<void> {
        if (
            !this.hasCapability(MatrixCapabilities.MSC3973UserDirectorySearch)
        ) {
            return this.transport.reply<IWidgetApiErrorResponseData>(request, {
                error: { message: "Missing capability" },
            })
    ...
    private async handleGetMediaConfig(
        request: IGetMediaConfigActionFromWidgetActionRequest,
    ): Promise<void> {
    ...
    private async handleDownloadFile(
        request: IDownloadFileActionFromWidgetActionRequest,
    ): Promise<void> {
        ...
        try {
            const result = await this.driver.downloadFile(
            ...
            this.handleDriverError(
                e,
                request,
                "Unexpected error while downloading a file",
            )
        }
    }
        ...
        if (!actionEv.defaultPrevented) {
            switch (ev.detail.action) {
                case WidgetApiFromWidgetAction.ContentLoaded:
                    return this.handleContentLoadedAction(
                        <IContentLoadedActionRequest>ev.detail,
                    )
                ...
                case WidgetApiFromWidgetAction.SendEvent:
                    return this.handleSendEvent(
                        <ISendEventFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.SendToDevice:
                    return this.handleSendToDevice(
                        <ISendToDeviceFromWidgetActionRequest>ev.detail,
                    )
                ...
                case WidgetApiFromWidgetAction.MSC2931Navigate:
                    return this.handleNavigate(
                        <INavigateActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC2974RenegotiateCapabilities:
                    return this.handleCapabilitiesRenegotiate(
                        <IRenegotiateCapabilitiesActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC2876ReadEvents:
                    return this.handleReadEvents(
                        <IReadEventFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.WatchTurnServers:
                    return this.handleWatchTurnServers(
                        <IWatchTurnServersRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.UnwatchTurnServers:
                    return this.handleUnwatchTurnServers(
                        <IUnwatchTurnServersRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC3869ReadRelations:
                    return this.handleReadRelations(
                        <IReadRelationsFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC3973UserDirectorySearch:
                    return this.handleUserDirectorySearch(
                        <IUserDirectorySearchFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.BeeperReadRoomAccountData:
                    return this.handleReadRoomAccountData(
                        <IReadRoomAccountDataFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC4039GetMediaConfigAction:
                    return this.handleGetMediaConfig(
                        <IGetMediaConfigActionFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC4039UploadFileAction:
                    return this.handleUploadFile(
                        <IUploadFileActionFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC4039DownloadFileAction:
                    return this.handleDownloadFile(
                        <IDownloadFileActionFromWidgetActionRequest>ev.detail,
                    )
                case WidgetApiFromWidgetAction.MSC4157UpdateDelayedEvent:
                    return this.handleUpdateDelayedEvent(
                        <IUpdateDelayedEventFromWidgetActionRequest>ev.detail,
                    )
     ...
     */
    public updateTheme(
        theme: IThemeChangeActionRequestData,
    ...
    public takeScreenshot(): Promise<IScreenshotActionResponseData> {
        return this.transport.send(
            WidgetApiToWidgetAction.TakeScreenshot,
            <IWidgetApiRequestEmptyData>{},
        )
    }
    ...
    public updateVisibility(
        ...
        return this.transport.send(WidgetApiToWidgetAction.UpdateVisibility, <
            IVisibilityActionRequestData
        >{
            visible: isVisible,
        })
    }
    ...
    public sendWidgetConfig(data: IModalWidgetOpenRequestData): Promise<void> {
        return this.transport
            .send<IModalWidgetOpenRequestData>(
                WidgetApiToWidgetAction.WidgetConfig,
                data,
            )
            .then()
    }

    public notifyModalWidgetButtonClicked(
        id: IModalWidgetOpenRequestDataButton["id"],
    ): Promise<void> {
        return this.transport
            .send<IModalWidgetButtonClickedRequestData>(
                WidgetApiToWidgetAction.ButtonClicked,
                { id },
            )
            .then()
    }
    ...
    public notifyModalWidgetClose(data: IModalWidgetReturnData): Promise<void> {
        return this.transport
            .send<IModalWidgetReturnData>(
                WidgetApiToWidgetAction.CloseModalWidget,
                data,
            )
            .then()
    }
    ...
    public async feedEvent(
        ...
        if (currentViewedRoomId !== undefined)
            this.setViewedRoomId(currentViewedRoomId)
        if (
            rawEvent.room_id !== this.viewedRoomId &&
            !this.canUseRoomTimeline(rawEvent.room_id)
        ) {
            return // no-op
        }
        ...
        if (rawEvent.state_key !== undefined && rawEvent.state_key !== null) {
            ...
            if (
                !this.canReceiveRoomEvent(
                    rawEvent.type,
                    (rawEvent.content as { msgtype?: string })?.["msgtype"],
                )
            ) {
                return // no-op
            }
    ...
    private pushRoomState(roomId: string): void {
        for (const cap of this.allowedEvents) {
            if (
                cap.kind === EventKind.State &&
                cap.direction === EventDirection.Receive
            ) {
                // Initiate the task
                const events = this.driver.readRoomState(
                    roomId,
                    cap.eventType,
                    cap.keyStr ?? undefined,
                )
                const task = events
                    .then(
                        (events) => {
                            ...
                            for (const event of events) {
                                let eventTypeMap =
                                    this.pushRoomStateResult.get(roomId)
                                if (eventTypeMap === undefined) {
                                    eventTypeMap = new Map()
                                    this.pushRoomStateResult.set(
                                        roomId,
                                        eventTypeMap,
                                    )
                                }
                                let stateKeyMap = eventTypeMap.get(
                                    cap.eventType,
                                )
                ...
                this.flushRoomStateTask.catch((e) =>
                    console.error("Failed to push room state", e),
                )
            }
        }
    }
    ...
    public async feedStateUpdate(rawEvent: IRoomEvent): Promise<void> {
        ...
        if (
            ...
            if (this.pushRoomStateTasks.size === 0) {
                ...
                let eventTypeMap = this.pushRoomStateResult.get(
                    rawEvent.room_id,
                )
                if (eventTypeMap === undefined) {
                    eventTypeMap = new Map()
